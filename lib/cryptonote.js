...
const Crypto = require('./traaittplatform-crypto')
const Mnemonic = require('./mnemonic')
const Numeral = require('numeral')
const SecureRandomString = require('secure-random-string')
const Transaction = require('./transaction')
const traaittPlatformCrypto = new Crypto()
...
class CryptoNote {
   ...
   */
  get cryptoType () {
    return traaittPlatformCrypto.type
  }
  ...
  underivePublicKey (derivation, outputIndex, outputKey) {
    ...
    const [err, key] = traaittPlatformCrypto.underivePublicKey(derivation, outputIndex, outputKey)
...
function cnFastHash (input) {
  ...
  const [err, hash] = traaittPlatformCrypto.cn_fast_hash(input)
...
function derivePublicKey (derivation, outputIndex, publicKey) {
  ...
  const [err, key] = traaittPlatformCrypto.derivePublicKey(derivation, outputIndex, publicKey)
...
function deriveSecretKey (derivation, outputIndex, privateKey) {
  ...
  const [err, key] = traaittPlatformCrypto.deriveSecretKey(derivation, outputIndex, privateKey)
...
function generateKeyDerivation (transactionPublicKey, privateViewKey) {
  ...
  const [err, derivation] = traaittPlatformCrypto.generateKeyDerivation(transactionPublicKey, privateViewKey)
...
function generateKeyImage (publicKey, privateKey) {
  ...
  const [err, keyImage] = traaittPlatformCrypto.generateKeyImage(publicKey, privateKey)
...
function checkRingSignatures (transactionPrefixHash, keyImage, inputKeys, signatures) {
  ...
  if (userCryptoFunctions.checkRingSignatures) {
    ...
    return traaittPlatformCrypto.checkRingSignatures(transactionPrefixHash, keyImage, inputKeys, signatures)
  }
}
...
function generateRingSignaturesPrimitive (transactionPrefixHash, keyImage, inputKeys, privateKey, realIndex) {
  ...
  if (userCryptoFunctions.generateRingSignatures) {
    ...
    const [err, sigs] = traaittPlatformCrypto.generateRingSignatures(transactionPrefixHash, keyImage, inputKeys, privateKey, realIndex)
...
function generateSignature (digestHash, publicKey, privateKey) {
  ...
  if (userCryptoFunctions.generateSignature) {
    ...
    [err, signature] = traaittPlatformCrypto.generateSignature(digestHash, publicKey, privateKey)
  }
...
function privateKeyToPublicKey (privateKey) {
  ...
  const [err, key] = traaittPlatformCrypto.secretKeyToPublicKey(privateKey)
...
function scReduce32 (hex) {
  const [err, result] = traaittPlatformCrypto.scReduce32(hex)
...
function verifySignature (digestHash, publicKey, signature) {
  ...
  return traaittPlatformCrypto.checkSignature(digestHash, publicKey, signature)
}
