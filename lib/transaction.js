...
class Transaction {
   ...
   */
  get hash () {
    const [err, hash] = traaittPlatformCrypto.cn_fast_hash(this.blob)

    /* Version 2 transactions are actualy double hashed to get the hash */
    if (this.version >= 2) {
      const [err, hash2] = traaittPlatformCrypto.cn_fast_hash(hash + TransactionVersion2Suffix)
   ...
   */
  get prefixHash () {
    const [err, hash] = traaittPlatformCrypto.cn_fast_hash(this.prefix)
